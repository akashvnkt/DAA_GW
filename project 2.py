...
print("The data has been written to 'quickselect_median_of_medians_analysis.csv'")

# Read the CSV file
data = pd.read_csv('project_2_file.csv')

# Extract columns for plotting
n_values = data['Values of n']
experimental_values = data['Experimental values (s)']
scaled_theoretical_values = data['Scaled theoretical values']

# Plotting the graph
plt.figure(figsize=(10, 6))
plt.plot(n_values, experimental_values, label='Experimental Time', marker='o', color='blue')
plt.plot(n_values, scaled_theoretical_values, label='Scaled Theoretical Time', linestyle='--', marker='x', color='red')

# Adding titles and labels
plt.title('Comparison of Experimental and Scaled Theoretical Time')
plt.xlabel('Values of n')
plt.ylabel('Time (seconds)')
plt.legend()
plt.grid(True, linestyle='--', alpha=0.7)

# Set x-axis to a logarithmic scale since n ranges widely
plt.xscale('log')
plt.yscale('log')
